...
int solve(int i=0,int j=n-1,int ops=0){
    if(ops>k) return OO;
    if(i>=j) return 0;
    if(mp.find(make_pair(make_pair(i,j),ops))!=mp.end()) return mp[make_pair(make_pair(i,j),ops)];
    if(arr[i]==arr[j]) return solve(i+1,j-1,ops);
    return mp[make_pair(make_pair(i,j),ops)]=min(solve(i+1,j,ops+1)+1,solve(i,j-1,ops+1)+1);
}
